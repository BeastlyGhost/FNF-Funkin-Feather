...
import fnf.objects.Stage;
import fnf.objects.ui.*;
import fnf.objects.ui.Note.BabyArrow;
import fnf.objects.ui.UI;
import fnf.song.*;
import fnf.song.MusicState;
import fnf.song.SongFormat.FeatherSong;
import fnf.states.substates.GameOverSubstate;
...
class PlayState extends MusicBeatState {
	...
	public static function set_song(newSong:FeatherSong):FeatherSong {
		if (newSong != null && song != newSong) {
			...
			if (strumsGroup != null && strumsGroup.members != null) {
				for (i in 0...strumsGroup.members.length) {
					// remove all notes from every strumline if they exist
					if (strumsGroup.members[i].allNotes != null) {
						var notes = strumsGroup.members[i].allNotes;
						notes.members.splice(0, notes.members.length);
					}
				}
			}
	...
	public override function create():Void {
		...
		strumsP1 = new Strum(strumX + (OptionsAPI.getPref("Center Notes") ? 0 : strumWidthX), strumY, assetSkin, [player], false,
			OptionsAPI.getPref("Downscroll"));
		strumsP2 = new Strum(strumX - strumWidthX, strumY, assetSkin, [opponent], true, OptionsAPI.getPref("Downscroll"));
	...
	public override function update(elapsed:Float):Void {
		...
		if (Controls.isJustPressed("pause") && canPause) {
			...
			openSubState(new fnf.states.substates.PauseSubstate(player.getScreenPosition().x, player.getScreenPosition().y, "default"));
		}
			...
			if (!isPaused && !hasDied && !isEndingSong) {
				if (gameplayMode != STORY) {
					...
					if (FlxG.keys.justPressed.EIGHT) {
						...
						Conductor.pauseSong();
						MusicState.switchState(new fnf.states.editors.OffsetEditor(char.name, char.player));
					}
				}
	...
	**/
	...
	public function noteHit(note:Note, babyStrum:Strum):Void {
		if (!note.noteData.wasGoodHit) {
			...
			if (babyArrow != null) {
				if (OptionsAPI.getPref("UI Style").toLowerCase() == "vanilla")
	...
	override function endSong():Void {
		...
		switch (gameplayMode) {
			case STORY:
				...
				MusicState.switchState(new fnf.states.menus.StoryMenu());
			case FREEPLAY:
				if (PlayerInfo.validScore)
					PlayerInfo.saveInfo(song.name, difficulty, dataSave, gameplayMode);
			...
			case CHARTING:
				...
				openSubState(new fnf.states.substates.PauseSubstate(player.getScreenPosition().x, player.getScreenPosition().y, "charting"));
		}
